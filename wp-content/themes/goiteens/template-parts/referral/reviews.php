<section class="section reviews">
  <div class="container">
    <h2 class="section-title reviews__title">
      <?php the_field('referral_reviews_title') ;?> 
    ...
    <ul class="list reviews__list grid reviews-js">
      ...
      <li class="reviews__item grid__item">
		...
		<?php $pathTop= get_field('referral_svg_baner_top');
		$height= get_field('referral_svg_baner_height');
		$pathBottom= get_field('referral_svg_baner_bottom')
		?>
			<?php get_template_part( 'template-parts/referral/components/svg', null, ['path' => $pathTop, 'height' => $height ]  ); ?> 
			</div>
           <?php if($item->parents_video_id):?>
          <div class="youtube" data-embed="<?php echo $item->parents_video_id; ?>"><?php get_template_part( 'template-parts/referral/components/play' ); ?></div>
           <?php else :?>
          <p class="reviews__item-description">
            <span class="reviews__item-text"><?php echo $item->parents_description; ?></span>
          </p>
           <?php endif; ?>
          <?php get_template_part( 'template-parts/referral/components/svg', null, ['path' => $pathBottom, 'height' => $height ]  ); ?> 
        </div>
